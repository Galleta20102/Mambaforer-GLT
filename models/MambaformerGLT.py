...
class mambaformerGLT(nn.Module):
    ...
    def detail_preservation_loss(self, output, content):
        content_features = self.encode_with_intermediate(content)[-1]
        output_features = self.encode_with_intermediate(output)[-1]
        
        # content region mask
        _, _, H, W = content_features.shape
        inner_mask = torch.ones_like(content_features)
        border = max(1, int(H * 0.05))
        
        inner_mask[:, :, :border, :] = 0.1
        inner_mask[:, :, -border:, :] = 0.1
        inner_mask[:, :, :, :border] = 0.1
        inner_mask[:, :, :, -border:] = 0.1
        
        detail_loss = F.mse_loss(output_features * inner_mask, content_features * inner_mask)
    ...
    def forward(self, samples_c: NestedTensor,samples_s: NestedTensor):
        ...
        detail_loss = self.detail_preservation_loss(Ics, content_input)
        
        # Please select and comment out one of the following two sentences
        return Ics,  loss_c, loss_s, loss_lambda1, loss_lambda2, detail_loss   #train
        # return Ics    #test 